 ...
 * - Apica (initialisation au démarrage)
 *
 * ============================================================
 */
...
public class CodexManager {
    ...
    public static void load(ResourceManager resourceManager) {
        ...
        loaded = true;
        Apica.LOGGER.info("Codex loaded: {} nodes total", NODES_BY_ID.size());
    }
    ...
    private static List<CodexNode> loadPage(ResourceManager resourceManager, CodexPage page) {
        ...
        ResourceLocation path = ResourceLocation.fromNamespaceAndPath(
            Apica.MOD_ID, "codex/" + page.getId() + ".json"
        );
        ...
        try {
            ...
            if (resourceOpt.isPresent()) {
                ...
                Apica.LOGGER.debug("Loaded {} nodes for page {}", nodes.size(), page.getId());
            } else {
                Apica.LOGGER.warn("Codex file not found: {}", path);
            }
        } catch (Exception e) {
            Apica.LOGGER.error("Failed to load codex page {}: {}", page.getId(), e.getMessage());
        }
