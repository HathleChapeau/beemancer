...
public class HoneyEnergyManager {
    ...
    private void consumeHoney() {
        ...
        if (!wasDepleted && honeyDepleted) {
            parent.notifyViewers(Component.translatable("gui.apica.network.honey_depleted"));
            parent.syncToClient();
        } else if (wasDepleted && !honeyDepleted) {
            parent.notifyViewers(Component.translatable("gui.apica.network.honey_restored"));
            parent.syncToClient();
        }
    }
}
