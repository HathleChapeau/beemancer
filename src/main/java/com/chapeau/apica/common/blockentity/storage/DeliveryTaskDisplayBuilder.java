...
public final class DeliveryTaskDisplayBuilder {
    ...
    static String computeBlockedReason(DeliveryTask task,
        ...
        if (honeyDepleted) return "gui.apica.tasks.blocked.honey_depleted";
        if (activeCount >= maxBees) return "gui.apica.tasks.blocked.no_bee_slot";
        if (!task.isReady(completedTaskIds)) return "gui.apica.tasks.blocked.dependency_pending";

        if (!task.isPreloaded()) {
            int available = containerOps.countItemInChest(task.getTemplate(), task.getSourcePos());
            if (available <= 0) return "gui.apica.tasks.blocked.items_unavailable";
        }
        ...
        if (!task.isPreloaded() && level != null && task.getSourcePos() != null
            ...
            if (be == null) return "gui.apica.tasks.blocked.target_missing";
        }
