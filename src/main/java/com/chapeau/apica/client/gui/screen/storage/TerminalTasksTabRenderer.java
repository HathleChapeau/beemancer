...
public class TerminalTasksTabRenderer {
    ...
    public void render(GuiGraphics g, Font font, StorageTerminalMenu menu,
        ...
        currentY = renderSection(g, font, x, currentY, mouseX, mouseY,
            "gui.apica.tasks.requests", 0xFFFFAA00,
        ...
        renderSection(g, font, x, currentY, mouseX, mouseY,
            "gui.apica.tasks.automation", 0xFF44FF44,
    ...
    private int renderSection(GuiGraphics g, Font font, int x, int currentY,
        ...
        if (roots.isEmpty()) {
            if (skipped >= taskScrollOffset && rendered < MAX_VISIBLE) {
                g.drawString(font, Component.translatable("gui.apica.tasks.empty"),
    ...
    private String getStateLabel(String state) {
        return switch (state) {
            case "FLYING" -> Component.translatable("gui.apica.tasks.state.flying").getString();
            case "QUEUED" -> Component.translatable("gui.apica.tasks.state.queued").getString();
            case "COMPLETED" -> Component.translatable("gui.apica.tasks.state.completed").getString();
            case "FAILED" -> Component.translatable("gui.apica.tasks.state.failed").getString();
            default -> state;
        };
    }
}
